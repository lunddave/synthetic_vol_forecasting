...
synth_vol_sim <- function(n, 
  ...
  for (i in 1:(n+1)){
    ...
    xreg_est <- round(coeftest(garch_1_1)[ dim(coeftest(garch_1_1))[1], 1],3)
    xreg_p_value <- round(coeftest(garch_1_1)[ dim(coeftest(garch_1_1))[1], dim(coeftest(garch_1_1))[2]],3)

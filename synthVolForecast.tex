...
\begin{enumerate}
  ...
  \item Let conditions Assumption 0 of \citet{han2014asymptotic} prevail.
\end{enumerate}
...
\begin{proof-of-proposition}
Recall the prediction function for the GARCH-X model:

$\E_{T^{*}}[\hat\sigma^{2}_{1,T^{*}+r}|\mathcal{F}_{T^{*}}] = \E[\hat\omega_{i} + \hat\omega^{*}_i D^{vol}_{i,t} + \sum^{m_{i}}_{k=1}\hat\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\hat\beta_{i,j}\sigma_{i,t-j}^{2} + \gamma_{i}^{T} \x_{i,t}]$

...
Note that under the popular GARCH(1,1), a dual level-volatility shock has an additive marginal effect on the conditional variance $\sigma^{2}_{i,t}$ that should be familiar to students of GARCH models: for $r\geq 2$, 

$$\sigma^{2 marginal}_{i,T^{*}+r} = \beta^{r-2}\big ( \alpha(\epsilon^{*}_{T^{*}+1})^{2} + \beta\omega^{*}_{i}\big )$$

Hence, the effect decays to 0 at a geometric rate. \\

\begin{proof}
By the specification (1) above, 

\begin{align}
&\sigma^{2}_{i, T^{*}+2} = \omega_{i} + \alpha(\epsilon^{*}_{T^{*}+1})^{2} + \beta\sigma^{2}_{i,T^{*}+1} = \omega_{i} + \alpha(\epsilon^{*}_{T^{*}+1})^{2} + \beta(\omega_{i} + \omega_{i}^{*} + \alpha\epsilon_{T^{*}}^{2} +\beta\sigma^{2}_{T^{*}}) \\
& \sigma^{2}_{i, T^{*}+3} = \omega_{i} + \alpha(\epsilon_{T^{*}+2})^{2} + \beta\sigma^{2}_{i,T^{*}+2} = \omega_{i} + \alpha(\epsilon_{T^{*}+2})^{2} + \beta(\omega_{i} + \alpha(\epsilon^{*}_{T^{*}+1})^{2} +\beta\sigma^{2}_{T^{*}+1})
\end{align}

From (4), we can see that both the level shock and the volatility shock persist in the volatility equation through only the final term, which has the coefficient $\beta$.  This phenomenon begins at $T^{*}+2$ for the volatility shock $\omega^{*}$, whereas for the level shock $\epsilon^{*}$, it begins one step later. 
\end{proof}

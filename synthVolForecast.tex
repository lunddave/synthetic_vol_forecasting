...
\begin{align*}
  \mc{M}_1 \colon \begin{array}{l}
    ...
    \omega_{i,t}^{*} = D^{vol}_{i,t}[\mu_{\omega^{*}}+\delta^{T}\x_{i,t}+ u_{i,t}],
  \end{array}
  \end{align*}

  \textbf{What are we trying to accomplish and accommodate here in specifying the shocks?}
  \begin{enumerate}
    \item Parsimony (e.g. if we don't need time-varying shocks, let's get rid of them)
    \item Realism (i.e. we want something that reasonably captures what happens)
    \item The specification should be harmonious with the estimation of the shocks.  The estimation should not be too wacky
    \item Asymptotic results - these specification should, if possible, allow the formal results to go through just fine
  \end{enumerate}

  \textcolor{red}{There is a potential problem here with the covariates varying with time.  The problem, briefly stated, is that if, during the volatility shock period, the covariates are not equal at each time point, then the shocks will not be equal in distribution, and it therefore becomes illegitimate to estimate the shocks with one single indicator variable.  We can nevertheless estimate the shocks with as many indicator variables as the volatility shock length is.  That seems complicated.  Another option is stipulating that for the entire length of the volatility shock, the `signal' of the shock is solely determined by the covariate values at T*+1.  One simple way to implement that is by simply defining the shock to depend on covariate v.T*+1.  In the first case, do we need to make any distributional assumptions about the covariates?}\\
with time-invariant error structure
  \begin{align*}
    ...
    \end{align*}
Let $\mc{M}_{0}$ denote the subclass of $\mc{M}_{1}$ models such that $\delta \equiv 0$.  Note that $\mc{M}_{0}$ assumes that $\omega^{*}_{i,t}$ have no dependence on the covariates and are i.i.d. with $\E[ \omega^{*}_{i,t}]=\mu_{\omega^{*}}$, where the lack of indices $i$ or $t$ on $\mu_{\omega^{*}}$ indicates that it is shared across donors and is time-invariant. Models $\mc{M}_{1}$ and $\mc{M}_{0}$ parameterize changing dynamics after a shock is observed (when $t \geq T_i^*+1$ for each series $i$). It is important to note that our modeling setup will additionally suppose that $\x_{i,t}$ will be observed before $\sigma_{i,t}^2$ is observed. 
...
We now turn to the next section, where $\textbf{V}_{t}$ is employed in a procedure to arrive at a forecast adjustment. We present two one-step-ahead forecasts for the time series under study. The first is the unadjusted GARCH forecast (denoted below as $\hat\sigma^{2}_{unadjusted}$). The second is the adjusted forecast, which differs by the additive term that is estimated via our prediction methodology (denoted below as $\hat\sigma^{2}_{adjusted}$). These forecasts are: 
...
\begin{align*}
  ...
  \text{F2: } & \hat\sigma^{2}_{adjusted,T^{*}+1} = \hat\E[\sigma^{2}_{1,T_{1}^{*}+1}|\mathcal{F}_{T^{*}}] + \hat\omega^{*}_{T^{*}+1} && = && \hat\omega_{i} + \sum^{m_{i}}_{k=1}\hat\alpha_{i,k}a^{2}_{i,T^{*}+1-k} + \sum_{j=1}^{s_{i}}\hat\beta_{i,j}\sigma_{i,T^{*}+1-j}^{2} + \hat\gamma_{i}^{T} \x_{i,t} + \hat\omega^{*}_{T^{*}+1} \text{ .}
\end{align*}

\textcolor{red}{We may want to suppress the time-indexation on the aggregated shock estimator here.}
...
\subsection{Excess Volatility Estimators}
    ...
    The problem of aggregating estimated donor shocks begins with the data constraints.  Taking the estimated shocks as a given, we observe the pair $(\{\hat\omega^{*}_{i,T^{*}+1}\}^{n+1}_{i=2},\{\textbf{v}_{i}\}^{n+1}_{i=2})$.  Let $\Delta^{n} = \{\pi \in \mathbb{R}^n: \sum_{i=1}^n \pi_i = 1, \pi_i \geq 0 \, i = 1,...,n\}$, then we wish to recover weights $\{\weight_{i}\}^{n+1}_{i=2} \in \Delta^{n}$ leading to favorable forecasting properties.  These weights are used to compute an aggregate shock estimate 
\begin{equation} \label{adjustment}
	  \hat\omega^{*}_{T^{*}+1} = \sum^{n+1}_{i=2}\weight_{i}\hat\omega^{*}_{i,T^{*}+1},
\end{equation}

\textcolor{red}{Here, we probably need a piecewise definition that basically says `adjust during the shock in the time series under study; otherwise, the adjustment is zero.'}
    which will be taken to be our forecast adjustment term.  Since the weights $\{\weight_{i}\}_{i=2}^{n+1}$ are computed using $\mathcal{F}_{T^{*}_{i}}$, the set $\{\weight_{i}\}_{i=2}^{n+1}$ is deterministic, $\textit{modulo}$ any stochastic ingredient in the numerical methods employed to approximate $\x_{1,T^{*}}$ using a convex combination of donor covariates.  We say more about the properties of the shocks $\omega^{*}_{i,T^{*}+1}$ in section $\ref{SVF_properties}$. 
...
\begin{align*}
\E[\sigma^{2}_{i,t+1}|\mathcal{F}_{t}] = \omega_{i} +  \sum^{m_{i}}_{k=1}\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\beta_{i,j}\sigma_{i,t-j}^{2} + \gamma_{i}^{T} \x_{i,t} + \omega^{*}_{i,t+1} \text{ .}
\end{align*}
By replacing parameters with their estimates, we arrive at the prediction 
\begin{align*}
\hat\sigma^{2}_{i,t+1}|\mathcal{F}_{t} = \hat\omega_{i} + \sum^{m_{i}}_{k=1}\hat\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\hat\beta_{i,j}\hat\sigma_{i,t-j}^{2} + \hat\gamma_{i}^{T} \x_{i,t} + \hat\omega^{*}_{i,t+1} \text { ,}
\end{align*}
which converges in probability to 
\begin{align*}
  \hat\sigma^{2}_{i,t+1}|\mathcal{F}_{t} = \omega_{i} + \omega^{*}_{i,t+1} + \sum^{m_{i}}_{k=1}\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\beta_{i,j}\sigma_{i,t-j}^{2} + \gamma_{i}^{T} \x_{i,t} + \hat\omega^{*}_{i,t+1}
  \end{align*}

as $t\rightarrow\infty$ by a simple application of Slutsky's Theorem.
\end{proof-of-proposition}

\textcolor{red}{We need to check back with \ref{sigma_consistency} to make sure that the estimated shocks make sense here.}

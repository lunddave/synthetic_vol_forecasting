...
Note that under the popular GARCH(1,1), a dual level-volatility shock has an marginal effect on the conditional variance $\sigma^{2}_{i,t}$ that should be familiar to students of GARCH models.  Assume that both the volatility shock $\omega^{*}$ and the level shock $\epsilon^{*}_{i}$ are of length one only, and for notational simplicity, we drop the $*$ notation from $\epsilon_{i}$.  Then for $r\geq 2$,
...
In (5) and (6), observe that $\omega_{i}^{*}$ and $\epsilon^{*}_{i}$ appear at most once each, through the term $\sigma^{2}_{T^{*}+r}$.  Hence, the total effect decays to 0 at a geometric rate of $\beta$.  We generalize this observation in the following proposition.

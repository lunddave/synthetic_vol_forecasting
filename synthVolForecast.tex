...
\begin{proof-of-proposition}[\ref{decay_prop}]
  ...
  \begin{proof-of-lemma}[\ref{lemma_ref}]
    ...
    The result for each $\hat\omega^{*}_{i,*}$ follows from the consistency proof of the QMLE in GARCH-X models, as established by \citet{han2014asymptotic}.  As for a convex combination of $\{\hat\omega^{*}_{i,*}\}$, consider the following argument, we have suppressed the time-indexation:

    \begin{align}
    \x_{1} &= \sum^{n+1}_{i=2}\weight_{i}\x_{i}\\
    \delta^{T}\x_{1} &= \delta^{T}\sum^{n+1}_{i=2}\weight_{i}\x_{i}\\
    &=\sum^{n+1}_{i=2}\pi_{i}(\omega_{i}^{*} - \mu_{\omega^{*}} - u_{i,t})\\
    \mu_{\omega^{*}} + \delta^{T}\x_{1} + \sum^{n+1}_{i=2}\pi_{i}u_{i,t} &= \sum^{n+1}_{i=2}\pi_{i}\omega_{i}^{*} \label{convex_comb_of_omega}
    \end{align}

    Now, since each $\hat\omega^{*}_{i}$ converges in probability to $\omega^{*}_{i}$, by Slutsky, any finite linear combination of $\hat\omega^{*}_{i}$ converges in probability to $\omega^{*}_{i}$.  Hence, $\sum^{n+1}_{i=2}\pi_{i}\hat\omega_{i}^{*}$ converges in probability to the right-hand side of (\ref{convex_comb_of_omega}).  By setting $u_{i,t}$ to zero for all shock points and all $i$, we then have $\omega_{1}^{*} = \mu_{\omega^{*}} + \delta^{T}\x_{1} $ on the LHS.  Without taking the $u_{i,t}$ to be zero at the shock points, however, we still have convergence in distribution.  For any collection of $n+1$ i.i.d variables, any convex combination of $n$ of them is equal in distribution to the one left out.  Therefore, the left-hand side of \ref{convex_comb_of_omega} is equal in distribution to $\omega_{1}$, as desired.

...
We will suppose that $\x_{i,t=T^*_i}$ and $\textbf{v}_{i,t=T^*_i}$ are observed before the shock is manifested in market activity on $y_{i,t=T^*_i}$.

In light of the foregoing, we can rewrite the GARCH framework we are interested in as such

$$\sigma_{i,t}^{2} = \omega_{i} + \sum^{m_{i}}_{k=1}\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\beta_{i,j}\sigma_{i,t-j}^{2} + \gamma_{i}^{T} \begin{bmatrix}
          \x_{i,t} \\
           \textbf{v}_{i,t}
           \end{bmatrix}, a_{i,t} = \sigma_{i,t}\epsilon_{i,t}$$
...
In this section we make a novel contribution to the synthetic control framework by constructing a profile of a time series volatility.
...
\begin{align}
\mc{M}_1 \colon \sigma^{2}_{i,t} = \omega_{i} + \omega^{*}_i D_{i,t}  + \sum^{m_{i}}_{k=1}\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\beta_{i,j}\sigma_{i,t-j}^{2} + \gamma_{i}^{T} \begin{bmatrix}
          \x_{i,t} \\
           \textbf{v}_{i,t}
           \end{bmatrix}, a_{i,t} = \sigma_{i,t}\epsilon_{i,t} \label{equation1}
\end{align}
...
\begin{align*}
  \phi_i &\simiid \mc{F}_{\phi} \text{ where } |\mc{F}_{\phi}| < 1, [\phi \text{ needs to be replaced with conditions on } \alpha, \beta \text{which will be simulated fixed parameters}]\\
...
\omega^{*}_i &\indep  \phi_i \indep \gamma_i \indep \varepsilon_{i,t}.
\end{align*}

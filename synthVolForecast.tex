...
In light of the foregoing, we can rewrite the GARCH model we are interested in as such
...
\label{modelsetup}
In this section, we will describe the postulated models for which 
post-shock aggregated estimators are provided. The basic structures of these models 
are the same for all time series in the analysis.  We first sharply distinguish between a volatility shock induced by a return series shock and a volatility shock directly affecting the volatility equation of a GARCH model, without mediation of the return series.
...
\subsubsection{Aggregation Mechanism}
    ...
    Contrast the foregoing with least-squares estimation for the weight vector $\vec{w}$:
...
For 1-step-ahead prediction of $\sigma^{2}_{t}$, consider the difference 

\begin{align}
  & QL(\hat\sigma_{t+1, unadjusted}^{2},\sigma^{2})-QL(\hat\sigma^{2}_{t+1, adjusted},\sigma^{2})\\
   & =(\frac{\hat\sigma^{2}_{t+1,unadjusted}}{\sigma^{2}} - \log{\frac{\hat\sigma^{2}_{t+1,adjusted}}{\sigma^{2}}} - 1) - (\frac{\hat\sigma^{2}_{t+1,adjusted}}{\sigma^{2}} - \log{\frac{\hat\sigma^{2}_{t+1,adjusted}}{\sigma^{2}}} - 1)\\
   & = \frac{\hat\sigma^{2}_{t+1,unadjusted}-\hat\sigma^{2}_{t+1,adjusted}}{\sigma^{2}} + \log{\frac{\hat\sigma^{2}_{t+1,adjusted}}{\hat\sigma^{2}_{t+1,unadjusted}}}\label{QL Loss Consistency}
\end{align}
...
\begin{align}
   &\frac{(\hat\omega + \hat\alpha a^{2} + \hat\beta\sigma^{2}_{t})-(\hat\omega + \hat\alpha a^{2} + \hat\beta\sigma_{t}^{2} + \hat\omega^{*})}{\sigma_{t+1}^{2}} + \log{\frac{\hat\omega + \hat\alpha a^{2} + \hat\beta\sigma_{t}^{2} + \hat\omega^{*}}{\hat\omega + \hat\alpha a^{2} + \hat\beta\sigma_{t}^{2}}}\label{QL Loss Consistency - GARCH(1,1)}\\
   & = \log{\frac{\hat\omega + \hat\alpha a^{2} + \hat\beta\sigma_{t}^{2} + \hat\omega^{*}}{\hat\omega + \hat\alpha a^{2} + \hat\beta\sigma_{t}^{2}}} - \frac{\hat\omega^{*}}{\sigma^{2}_{t+1}} \label{QL Loss Consistency - GARCH(1,1)} 
\end{align}

Recall from Proposition $(\ref{consistency})$ that $\hat\omega \xrightarrow{p} \E[\omega]$ FIX.  Hence whether the adjusted forecast has smaller loss will depend upon how large $\omega$ is.  

The first term in $(\ref{QL Loss Consistency - GARCH(1,1)})$ will be nonnegative so long as $\hat\omega^{*}$ is nonnegative.
\subsection{Uniqueness of Convex Combinations}
\url{https://encyclopediaofmath.org/wiki/Chebyshev_set}
...
\subsection{Model Adjustment Using Synthetic Methods}
Here we distinguish 

...
On the questions of convex geometry and convex optimization, comparison with least-squares estimation is also illustrative.  Consider least-squares for the $p$-vector $\delta$
...
$$ \text{MSE}^{1}_{\text{aggregated predictor, RV}} = (\hat\sigma^{2}_{\text{aggregated predictor}} - \hat\sigma^{2}_{RV})^{2}$$
Also of interest in absolute percentage error for an $h$-step-ahead forecast, defined as

\[ 
\text{APE}^{h}_{\text{method, ground truth}} = \frac{|\hat\sigma^{2}_{h, \text{method}} - \hat\sigma^{2}_{h, \text{ground truth}}|}{\hat\sigma^{2}_{h, \text{ground truth}}}
\]
Finally, we introduce the QL (quasi-likelihood) Loss \citep{brownlees2011practical}:

\[ 
\text{QL}^{h}_{\text{method}, \text{ground truth}} = \frac{\hat\sigma^{2}_{h, \text{ground truth}}}{ \hat\sigma^{2}_{h, \text{method}}} - \log{\frac{\hat\sigma^{2}_{h, \text{ground truth}}}{ \hat\sigma^{2}_{h, \text{method}}}} -1 \text{ .}
\]

...
We are interested in a point forecast for $\sigma^{2}_{1,T^{*}+1}$, the 1-step ahead conditional variance for the time series under study.  The forecast performance is using three distinct loss functions, each computed using three families of estimators for the ground truth that we seek.  Let $L^{h}$ with the subscript (method, ground truth) denote the loss function L and a prediction method - ground truth estimator pair.  For example, a loss function of interest in this study is the 1-step-ahead MSE

$$ MSE^{1}_{synth pred, RV} = (\hat\sigma^{2}_{synth pred} - \hat\sigma^{2}_{RV})^{2}$$

In more generality, for an h-step-ahead volatility forecast, the loss function is 

$$ MSE^{h}_{method, ground truth} = \frac{1}{h}\sum_{g=1}^{h}(\hat\sigma^{2}_{g, method} - \hat\sigma^{2}_{g, ground truth})^{2}$$

Also of interest in mean absolute-percentage-error for an h-step-ahead forecast, defined as

\[ 
\text{MAPE}^{h}_{method, ground truth} = \frac{1}{h}\sum_{g=1}^{h}\frac{|\hat\sigma^{2}_{g, method} - \hat\sigma^{2}_{g, ground truth}|}{\hat\sigma^{2}_{g, ground truth}}
\]

Finally, we introduce the QL (quasi-likelihood) Loss \citep{brownlees2011practical}:

\[ 
\text{QL}^{h}_{method, ground truth} = \frac{1}{h}\sum_{g=1}^{h} \frac{ \hat\sigma^{2}_{g, method} }{\hat\sigma^{2}_{g, ground truth}} - \log{\frac{ \hat\sigma^{2}_{g, method} }{\hat\sigma^{2}_{g, ground truth}}} -1
\]
...
\begin{align}
\sigma^{2}_{i,T^{*}+r+1} & = \omega_{i} + \alpha a_{i,T^{*}+r}^{2} + \beta\sigma^{2}_{i,T^{*}+r} \\
...
\end{align}
 ...
 where the added random/fixed effects are
...
% \mathbf{v}_i &\simiid \mc{F}_{\mathbf{v}} \text{ with }  \; \mrm{E}_{\mc{F}_{\mathbf{v}}}(\mathbf{v}_i) = \mu_{\mathbf{v}}, \mrm{Var}_{\mc{F}_{\mathbf{v}}}(\mathbf{v}_i)  = \Sigma^2_{\mathbf{v}} \\
\varepsilon_{i,t} & \simiid  \mc{F}_{\varepsilon} \text{ with }  \; \mrm{E}_{\mc{F}_{\varepsilon}}(\varepsilon_{i,t}) = 0, \mrm{Var}_{\mc{F}_{\varepsilon}}(\varepsilon_{i,t})  = 1 \\
\t\varepsilon_{i} & \simiid  \mc{F}_{\t\varepsilon} \text{ with }  \; \mrm{E}_{\mc{F}_{\t\varepsilon}}(\t\varepsilon_{i}) = 0, \mrm{Var}_{\mc{F}_{\varepsilon}}(\varepsilon_{i,t})  = 1 \\
%l_{i,vol} &\simiid \text{Unif}\{1,2,...,\text{maximum volatility shock length}\}\\
%l_{i, level} &\simiid \text{Unif}\{1,2,...,l_{i,vol}\}\\
\omega^{*}_i &\indep  (\alpha, \beta)_i \indep \gamma_i \indep \varepsilon_{i,t}.
\end{align*}

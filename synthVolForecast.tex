...
\begin{align*}
  ...
  \epsilon_i &\simiid \mc{F}_{\epsilon} \text{ with }  \; \mrm{E}_{\mc{F}_{\epsilon}}(\epsilon) = 0, \mrm{Var}_{\mc{F}_{\epsilon}}(\epsilon)  = \sigma^2_{\epsilon}  \\
  ...
  \omega^{*}_{i} &\indep \epsilon_i \indep \epsilon^{*}_i  \indep \varepsilon_{i,t}.
  \end{align*}
...
\begin{align*}
  ...
  \begin{align*}
    ...
    %l_{i,vol} &\simiid \text{Unif}\{1,2,...,\text{maximum volatility shock length}\}\\
    %l_{i, level} &\simiid \text{Unif}\{1,2,...,l_{i,vol}\}\\
    \omega^{*}_i & \indep  \epsilon_i \indep  \epsilon^{*}_i \indep \delta \indep \varepsilon_{i,t}
    \end{align*}
...
\subsection{Consistency of the Synthetic Volatility fixed-effect estimators and Synthetic Volatility prediction functions}



Assume
\begin{enumerate}[\itshape(a)]
  \item $\forall i$, $\{a_{t}\}_{t=1,...,T_i}$ obeys a GARCH($m,s$) process, where $T_i$ is the length of the $i$-th series.
  \item $\{X_{i,t}\}$
  \item Let conditions x,y,z in \citet{han2014asymptotic}
\end{enumerate}

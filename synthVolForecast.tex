...
\subsubsection{The Signal-to-Noise Question}
    ...
    \end{subfigure}
        ...
        \caption{In the progression from (a) to (b) to (c), we see that increasing $\delta$ leads to improved performance of the adjusted forecast, and this effect is intensified as $\mu_{x}$ increases.  However, it is not consistently true that an increasing noise undermines the performance of the adjusted forecast.  Rather, that phenomenon requires both large quantites of $\delta$ and $\mu{x}$.}
        \label{fig:signoise}
      \end{figure}
...
\subsubsection{The Role of the Covariate X mean}
  ...
  \begin{figure}[!h]
    \centering
    \textbf{The relation between the mean of X and the signal}\par\medskip
  \begin{subfigure}{.44\linewidth} 
    \centering
  ...
  \end{subfigure}
      ...
      \caption{the whole thing is over}
      \label{fig:signoise}
    \end{figure}
...
\begin{figure}[!h]
  \centering
  \textbf{The relation between the intercept of the shock and the noise}\par\medskip
\begin{subfigure}{.44\linewidth} 
  \centering
...
\begin{subfigure}{.44\linewidth} 
  \centering
    ...
    \caption{Fixed parameter values: $\omega = .2, \alpha = .1, \beta = .82, \mu_{x} = .125, \sigma_{x} = .125, \delta = .125$}\label{fig:sim_7}
\end{subfigure}

...
\textcolor{green}{It looks like they've had periods/commas added to them, but let me know if you see any still in need of punctuation.}
...
\end{align*}
Let $I(\cdot)$ be an indicator function, $T_i$ be the time length of the time series $i$ for $i = 1, \ldots, n+1$, and $T_i^*$ denote the largest time index prior to the arrival of the news shock , with $T_i^* < T_i$.  For $t= 1, \ldots, T_i$ and $i = 1, \ldots, n+1$, the model $\mc{M}_1$ is defined as follows.  
\begin{align*}
  \mc{M}_1 \colon \begin{array}{l}
    ...
    \omega_{i,t}^{*} = \mu_{\omega^{*}}+\delta'\mbf{x}_{i, t-1}+ u_{i,t}
  \end{array}
  \end{align*}

  The random effects structure is as follows.
  ...
  \begin{align*}
    ...
    \end{align*}
where $D^{level}_{i,t} = I(t \in \{T_i^* + 1,...,T_i^* + L_{i, level}\})$ and $L_{i,vol},L_{i,level}$ denote the lengths of the volatility and level shocks, respectively. Let $\x_{i,t} \in \mathbb{R}^{p}$.  Let $\mc{M}_{0}$ denote the subclass of $\mc{M}_{1}$ models such that $\delta \equiv 0$.  Notice that $\mc{M}_{0}$ assumes that $\omega^{*}_i$ have no dependence on the covariates and are i.i.d. with $\E[ \omega^{*}_i]=\mu_{\omega^{*}}$.  We further define the filtrations \textcolor{green}{We might not need to define these.  I rarely see filtrations defined.}
    \begin{align*}
      \begin{array}{lll}
         \mathcal{F}_{t}^{\mc{M}_{1}} &= &\{(\x_{i,l}, \omega^{*}_i, \epsilon_{i,l}, \epsilon^{*}_{i,l}, \varepsilon_{i,l})\colon l\leq t\}\\
         \mathcal{F}_{t}^{\mc{M}_{2}} &= &\{(\x_{i,l}, \omega^{*}_i, \epsilon_{i,l}, \epsilon^{*}_{i,l},  \varepsilon_{i,t})\colon l \leq t\}\\
        \end{array}
    ...
    \begin{equation*}
      ...
      \end{pmatrix}
      \end{equation*}
      Several comments are in order here.  Although as shown, $\textbf{V}_{p,n}$ displays `balance' in that each of the $p$ covariates is replicated $n$ times, in practice, it may be that some donors do not possess or do not require such information. $\textbf{V}_{p,n}$ \textcolor{red}{more comments are needed?}\textcolor{green}{Agreed on both comments in this section.  I may need a section or at least a mention of how the \citet{abadie2010synthetic} nests a GARCH model.  After having done that, the volatility profile will make more sense, and it will be more justifiable to call it a volatility profile, I hope, than just a matrix of covariates.}
...
\begin{align*}
  ...
  \text{Forecast 2: }  & \hat\sigma^{2}_{adjusted}  \hat\E_{T^{*}}[\sigma^{2}_{i,T^{*}+1}|\mathcal{F}_{T^{*}}] + \hat\omega^{*} = \hat\omega_{i} + \sum^{m_{i}}_{k=1}\hat\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\hat\beta_{i,j}\sigma_{i,t-j}^{2} + \hat\gamma_{i}^{T} \x_{i,t} + \hat\omega^{*} 
\end{align*}
...
We are interested in a point forecast for $\sigma^{2}_{1,T^{*}+h}$, $h=1,2,...,H$, the $h$-step ahead conditional variance for the time series under study, up to a forecast length of $H$.  \textcolor{red}{The forecast performance evaluation uses three distinct loss functions, each computed using three families of estimators for the ground truth that we seek.  This should change, since we are using just QL in the paper.}.  Let $L^{h}$ with the subscripted pair $\{$prediction method, ground truth estimator$\}$, denote the loss function for an $h$-step-ahead forecast using a given prediction function and ground truth estimator.  For example, one loss function of interest in this study is the 1-step-ahead MSE using Synthetic Volatility Forecasting and Realized Volatility:
...
\begin{enumerate}
  ...
  \item For each $i, \textcolor{red}{\{\omega_{i,t}^{*}\}_{t=0,...,T_i}}$ is potentially non-zero at $\{T^{*}_{i}+1,... ,T^{*}_{i}+k\}$, $\omega_{i,T^{*}+1}^{*}\equiv...\equiv\omega_{i,T^{*}+k}^{*}$, and zero otherwise, where the arrival of $T_{i}^{*}$ is governed by a time-invariant distribution on $\{a_{i,t}\}_{t=0,...,T_i}$. \label{stationarity_of_omega_i_t}
...
\begin{proof-of-proposition}
The result follows from the consistency proof of the QMLE in GARCH-X models, as established by \citet{han2014asymptotic}. \textcolor{red}{Is this the proof of the Proposition or of the Lemma? I am very confused here.}\textcolor{green}{Proof of lemma now added.}
\end{proof-of-proposition}
...
\begin{proof-of-proposition}
  First, note that the function $g:(-\infty,\E[\omega_{i}^{*}])\rightarrow \mathbb{R}$ given by $g(x) = \log{\frac{\sigma^{2}_{t+1}}{\sigma_{t+1}^{2}-x}} - \frac{x}{\sigma^{2}_{t+1}}$ is nonnegative, and being continuous, $g$ preserves consistency. The conclusion follows from fact that the model is correctly specified and consistency is guaranteed Proposition ($\ref{sigma_consistency}$). \textcolor{red}{Proof appears to not be done. Although it looks like it is almost done.}\textcolor{green}{Should Propositions and equations referred to have parentheses around the number?}
\end{proof-of-proposition}
...
\section{Supplement}
We analyze the real-world example with Brexit included and using the daily log-return in the Pound-Dollar exchange rate as a covariate. \\

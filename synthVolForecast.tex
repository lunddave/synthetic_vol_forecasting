...
\begin{align}
  & QL(\hat\sigma_{t+1, unadjusted}^{2},\sigma^{2})-QL(\hat\sigma^{2}_{t+1, adjusted},\sigma^{2})\\
   & =(\frac{\hat\sigma^{2}_{t+1,unadjusted}}{\sigma^{2}} - \log{\frac{\hat\sigma^{2}_{t+1,unadjusted}}{\sigma^{2}}} - 1) - (\frac{\hat\sigma^{2}_{t+1,adjusted}}{\sigma^{2}} - \log{\frac{\hat\sigma^{2}_{t+1,adjusted}}{\sigma^{2}}} - 1)\\
...
\end{align}
By letting $f(\hat\omega^{*}) =\log{\frac{\hat\omega + \hat\alpha a^{2} + \hat\beta\sigma_{t}^{2} + \hat\omega^{*}}{\hat\omega + \hat\alpha a^{2} + \hat\beta\sigma_{t}^{2}}} - \frac{\hat\omega^{*}}{\sigma^{2}_{t+1}}$, it is easily verified that $f$ is minimized at $\hat\omega^{*} = 0$, i.e., as the adjusted forecast converges to the unadjusted.  Similarly, the loss of the unadjusted forecast will be greater, so long as the estimated adjustment term $\hat\omega^{*}$ is in appropriate proportion to the actual volatility $\sigma^{2}_{t+1}$.  This explains why it is so important to avoid using a naive adjustment estimator like the arithmetic mean of the estimated shocks.  
...
\begin{enumerate}
  \item k-dimensional thing to predict

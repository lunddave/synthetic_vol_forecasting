...
\begin{align*}
  %\text{Forecast 1: } 
  & \hat\sigma^{2}_{unadjustedT_{1}^{*}+1} = \hat\E[\sigma^{2}_{1,T_{1}^{*}+1}|\mathcal{F}_{T_{1}^{*}}] && = && \hat\omega_{1} + \sum^{m_{1}}_{k=1}\hat\alpha_{1,k}a^{2}_{1,T_{1}^{*}+1-k} + \sum_{j=1}^{s_{1}}\hat\beta_{1,j}\sigma_{1,T_{1}^{*}+1-j}^{2} + \hat\gamma_{1}^{T} \x_{1,T_{1}^{*}+1},\\
  %\text{Forecast 2: } 
...
Comparison with least-squares estimation is also illustrative.  Consider least-squares for the $p$-vector $\delta$

\begin{align*}
 \hat\delta=\argmin_{\delta}\|\hat{\omega}^{*} - \delta^{T}\textbf{V}_{T^{*}+1}\|^{2}_{2},
\end{align*}
...
$$ \text{MSE}^{1}_{\text{aggregated predictor, RV}} = (\hat\sigma^{2}_{aggregated predictor} - \hat\sigma^{2}_{RV})^{2}$$
...
$$QL(\hat\sigma_{unadjusted,t+1}^{2},\sigma^{2}_{t+1})-QL(\hat\sigma^{2}_{adjusted,t+1},\sigma^{2}_{t+1}) \overset{p}{\longrightarrow} \frac{\omega_{i,t+1}^{*}}{\sigma^{2}_{t+1}-\omega_{i,t+1}^{*}} + \log{\frac{\sigma_{t+1}^{2}-\omega_{i,t+1}^{*}}{\sigma_{t+1}^{2}} } \geq 0.$$

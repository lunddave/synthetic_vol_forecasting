...
\subsubsection{Aggregation Mechanism}
    ...
    There is no guarantee that $\vec{w}_{OLS}$ would perform poorly as a tool for producing $\hat\omega = X_{1,T^{*}}$, but given the small number of donors supposed in our setting, it is risky.
...
\begin{align}
   &\frac{(\hat\omega + \hat\alpha a_{t}^{2} + \hat\beta\sigma^{2}_{t})-(\hat\omega + \hat\alpha a_{t}^{2} + \hat\beta\sigma_{t}^{2} + \hat\omega^{*})}{\sigma_{t+1}^{2}} + \log{\frac{\hat\omega + \hat\alpha a_{t}^{2} + \hat\beta\sigma_{t}^{2} + \hat\omega^{*}}{\hat\omega + \hat\alpha_{t} a^{2} + \hat\beta\sigma_{t}^{2}}}\label{QL Loss Consistency - GARCH(1,1)}\\

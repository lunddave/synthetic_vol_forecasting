...
In order to motivate our procedure, we show how the aggregation of estimated excess volatilities from donors in the donor pool works when the correction function is the arithmetic mean.
...
\subsubsection{Aggregation Mechanism}
    ...
    The problem of aggregating estimated donor shocks begins with the data constraints.  Taking the estimated shocks as a given, we essentially observe the pair $(\{\hat\omega_{i}\}^{n+1}_{i=2},\{x_{i,T^{*}}\}^{n+1}_{i=2})$.  We wish to recover weights $\{w_{i}\}^{n+1}_{i=2} \in \Delta^{n}$ that will minimize our loss.  Let $L$ denote any loss function, not necessarily symmetric, mapping two quantities to the non-negative reals.  Formally, our optimization problem is \\ 

...
\subsection{Unbiasedness of the Synthetic Volatility fixed-effect estimators and Synethetic Volatility prediction functions}
...
\begin{proof-of-proposition}
First, for each i,t, define $\eta_{i,t} \coloneqq a^{2}_{i,t} - \sigma^{2}_{i,t}$, and by adding that to both sides of the conditional variance specification of $\mc{M}_1$ and $\mc{M}_2$, we can obtain the standard ARMA representation of a GARCH(m,s):
...
a^{2}_{i,t} = a^{2}_{i,t} - \sigma^{2}_{i,t} + \sigma^{2}_{i,t} = \eta_{i,t} + \sigma^{2}_{i,t} &= \eta_{i,t} + \omega_{i} + \omega^{*}_i D_{i,t}  + \sum^{m_{i}}_{k=1}\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\beta_{i,j}\sigma_{i,t-j}^{2} + \gamma_{i}^{T} \x_{i,t} \\
    &= \eta_{i,t} + \omega_{i} + \omega^{*}_i D_{i,t}  + \sum^{m_{i}}_{k=1}\alpha_{i,k}a^{2}_{i,t-k}  + \sum_{j=1}^{s_{i}}\beta_{i,j}(a^{2}_{i,t-k} - \eta_{i,t-k}) + \gamma_{i}^{T} \x_{i,t} \\
    &= \eta_{i,t} + \omega_{i} + \omega^{*}_i D_{i,t}  + \sum^{m_{i}}_{k=1}(\alpha_{i,j} + \beta_{i,j})a^{2}_{i,k} - \sum_{j=1}^{s_{i}}\beta_{i,j}\eta_{i,t-j} + \gamma_{i}^{T} \x_{i,t} \\
\end{align*}

Next, as shown in \citet{abadie2010synthetic}, under the autoregressive model

\begin{align}
\mc{M}_3 \colon \begin{array}{l}
  Place Abadie model here
\end{array}\label{model2}
\end{align}

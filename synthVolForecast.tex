...
\begin{align*}
\delta_{t} & \sim \omega,  \textbf{a location parameter shared across donors}\\
\boldsymbol\theta_{t} & \sim \boldsymbol\alpha_{k},  \textbf{the ARCH parameters shared across donors} \\
\textbf{Z}_{i} & \sim \boldsymbol a_{t-k}, \textbf{observable quantities specific to each donor} \\
\boldsymbol \lambda_{t} & \sim \boldsymbol\beta_{j}, \textbf{the GARCH parameters shared across donors} \\
\boldsymbol \mu_{i} & \sim \boldsymbol \sigma_{i,t-j}^{2}, \textbf{latent quantities specific to each donor}   \\
\end{align*}
    ...
    \begin{equation*}
      ...
      \end{pmatrix}
      \end{equation*}
      Several comments are in order here.  Although as shown, $\textbf{V}_{p,n}$ displays `balance' in that each of the $p$ covariates is replicated $n$ times, in practice, it may be that some donors do not possess or do not require such information. $\textbf{V}_{p,n}$
...
\section{Supplement}
We analyze the real-world example with Brexit included and using the daily return in the Pound-Dollar exchange rate as a covariate. \\

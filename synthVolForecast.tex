...
\item Use the result (3.14) from Tsay p. 93, especially in the case of the a pure volatility shock, in which case $\omega^{*}_{i}$ shows up in numerator of expectation.
\item Emphasize that $\epsilon^{*}_{i,t}$ is a nuisance paramter.  However, there is an important feature of the $\epsilon^{*}_{i,t}$ that requires more thought: does the variance need to be small in order for the estimation of $\omega^{*}_i$ to work?
\end{enumerate}

\subsection{Factors (parameters) to vary in simulations}
\begin{enumerate}
\item Size of donor pool, n: n = c(10,20,40,80,160,640)
\item Size of $(\alpha, \beta)$.
...
\item Based on everything above, the number of simulations would be sum(c(10,20,40,80,160,640)) * $(\alpha, \beta)$ * 3 * 3 * 6 = 51,300.
\end{enumerate}

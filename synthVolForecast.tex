...
\subsection{Consistency of the shock estimates}
...
\end{enumerate}
Then for any $i$, $\hat\omega_{i,T^{*}+1}^{*} \overset{p}{\longrightarrow} \omega_{i,T^{*}+1}^{*}$ as $t\rightarrow\infty$.
\end{prop}
...
Having established the consistency of the estimators $\hat\omega^{*}_{i,T^{*}+1}$, we extend that result to prove the consistency of the conditional forecast function itself.
  ...
  \begin{prop}\label{sigma_consistency}
    ...
    \begin{enumerate}
     ...
     \end{enumerate}
  Then $\hat\sigma^{2}_{adjusted}\overset{p}{\longrightarrow}\sigma^{2}_{1,T^{*}+1}$ as $t\rightarrow\infty$. 
  \end{prop}

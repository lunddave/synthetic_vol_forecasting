...
and $\varepsilon_{i,t}$ is idiosyncratic noise, uncorrelated across time and donors.
    ...
    In this section, we make a novel contribution to prediction via distance-based weighting by constructing a profile of a time series' volatility, the analogue of a covariate matrix in a synthetic control framework. \textcolor{red}{What distinguishes a volatility shock profile is that -- ALSO, how I index time?} .  Suppose that for each of the $n$ donors, we posit $p$ distinct covariates in the functional form of the shock.  The volatility profile is defined as the $pn \times n$ matrix 

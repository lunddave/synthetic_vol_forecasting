...
Comparison with least-squares estimation is illustrative.  Consider least-squares for the weight vector $\vec{w}$:
...
There is no guarantee that $\vec{w}_{OLS}$ would perform poorly as a tool for producing $\hat\omega^{*}$, but given the small number of donors supposed in our setting, it is risky. \textcolor{red}{I think this should be removed or mentioned as a comment in the section where weights are introduced.}\textcolor{green}{I will think about where to put this.  The deep point that could be made, and perhaps should be made, is that finding an optimal or at least suitable convex combination of the donors works because of the assumptions about the DGP, and those assumptions don't necessarily support OLS much.}
    ...
    Suppose we examine $K$ units of of time, where each unit is divided into $m$ intervals of length $\frac{1}{m}$.  We follow the notation of \citet{andersen2009realized}. Let $p_{t} = \log{P_{t}}$, and let $\tilde{r}(t,\frac{1}{m}) = p_{t} - p_{t-\frac{1}{m}}$.  We estimate the variance of $i$th log return series using Realized Volatility of the $K$ consecutive trading days that conclude with day $t$, denoted $RV_{i,t}^{K,m}$, using
    
    $$RV^{K,m} = \frac{1}{K}\sum^{Km}_{v=1}\tilde{r}^{2}(v/m,1/m).$$
...
Assume
\begin{enumerate}
  \item For each $i$, $\{a_{i,t}\}_{t=0,...,T_i}$ obeys a GARCH-X($m,s$), as laid out in equation $(\ref{GARCH-X})$, with volatility shocks found in $\mc{M}_{2}$, where $T_i$ is the length of the $i$th series.
...
\begin{comment}
To see this, for each $i,t$, define $\eta_{i,t} \coloneqq a^{2}_{i,t} - \sigma^{2}_{i,t}$, and by adding $\eta_{i,t}$ to both sides of the conditional variance specification of $\mc{M}_{0}$ and $\mc{M}_{1}$, we obtain the standard ARMA representation of a GARCH(m,s):
...
The proposed method is unlike KNN in that are not trying to learn a function, first and foremost.  We are trying to estimate a parameter.
...
We conclude our discussion by proposing a subfield of forecasting based on the common methodologies of this work and \citet{lin2021minimizing}.  We begin by distinguishing what in this work is essential and inessential to synthetic forecasting.

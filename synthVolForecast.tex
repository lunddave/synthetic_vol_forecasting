...
\begin{align*}
  ...
  \textcolor{red}{There is a potential problem here with the covariates varying with time.  The problem, briefly stated, is that if, during the volatility shock period, the covariates are not equal, then the shocks will not be equal in distribution, and it therefore becomes illegitimate to estimate the shocks with one single indicator variable.  We can nevertheless estimate the shocks with as many indicator variables as the volatility shock length is.  That seems complicated.  Another option is stipulating that for the entire length of the volatility shock, the `signal' of the shock is solely determined by the covariate values at T*.  One simple way to implement that is by simply defining the shock to depend on covariate v.T*.  In the first case, do we need to make any distributional assumptions about the covariates?}\\

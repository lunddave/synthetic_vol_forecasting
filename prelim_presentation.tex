...
Goal: recover weights $\{\weight_{i}\}^{n+1}_{i=2} \in \Delta^{n}$ 
\bigbreak

Compute: $\hat\omega^{*} \coloneq \sum^{n+1}_{i=2}\weight_{i}\hat\omega^{*}_{i}$, our forecast adjustment term.  

\bigbreak

the set $\{\weight_{i}\}_{i=2}^{n+1}$ is deterministic
...
Suppose we examine $K$ units of of time, where each unit is divided into $m$ intervals of length $\frac{1}{m}$.  Let $p_{t} = \log{P_{t}}$, and let $\tilde{r}(t,\frac{1}{m}) = p_{t} - p_{t-\frac{1}{m}}$  \parencite[][]{andersen2009realized}. 

...
Credible forecasting is possible under news shocks, so long as we incorporate external information to account for the nonzero errors.
...
Consider the preceding with the factor model from \cite[][]{abadie2010synthetic}, where an untreated unit is governed by:
...
\begin{equation*}
    ...
    \end{pmatrix},
    \end{equation*}
    where RV denotes realized variance, IV the implied volatility

\end{frame}

\begin{frame}
    \frametitle{Significance of the Volatility Profile}
    Covariates chosen for inclusion in a given volatility profile may be \begin{itemize}
        \item levels
        \item differences in levels
        \item log differences in levels
        \item percentage changes in levels
...
Goal: recover weights $\{\weight_{i}\}^{n+1}_{i=2} \in \Delta^{n}$ and $\hat\omega^{*} \coloneq \sum^{n+1}_{i=2}\weight_{i}\hat\omega^{*}_{i}$, our forecast adjustment term, where set $\{\weight_{i}\}_{i=2}^{n+1}$ is deterministic with respect to $\mathcal{F_{T^{*}}}$.
...
\item RV can be decomposed into the sum of a continous component and a jump component, with the latter being less predictable and less persistent \parencite[][]{andersen2007roughing}, cited in \cite[][]{de2006forecasting}, two factors that further motivate the method employed herein.
\end{itemize}
...
Goal: point forecasts for $\sigma^{2}_{1,T^{*}+h}|\mathcal{F}_{T^{*}}$, $h=1,2,...,H$, the $h$-step ahead conditional variance for the time series under study, up to a forecast length of $H$.\\
...
This has benefits, both practical and theoretical.  \\

As \cite[][]{brownlees2011practical} explains, ``[a]mid volatility turmoil, large MSE
losses will be a consequence of high volatility without necessarily corresponding to deterioration of forecasting ability. The QL avoids this ambiguity, making it easier to compare losses across volatility regimes."  For this reason, we proceed to evaluate the method, both in simuations and real data examples, using the QL loss.
...
\begin{frame}{2016 Election}
    \begin{figure}[H]
        \begin{center}
          \includegraphics[scale=.2]{real_data_output_plots/savetime_SunMar172252462024_IYG_CL=F-^VIX-^IRX-^FVX-^TNX-^TYX_^VIX_2016-11-08-2004-11-02-2008-11-04-2012-11-06.png}
...
\begin{frame}
    ...
    If we switch the values of $\alpha$ and $\beta$, we see similar behavior, as in \ref{fig:heavy_alpha}.  
    \begin{figure}[h!]
      \begin{center}

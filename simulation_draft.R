...
extra_measurement_days <- c(0) #c(0,1,2,3,4)
a <- 3*252
b <- 10*252
replication_number <- seq(1, nsim, 1)
optimization_norm <- c('l1','l2')[2]
mu_eps_star <- 0 #seq(0,-1,-.1)
...
mu_omega_star <- seq(.0,.6, .3) 
vol_shock_sd <- seq(0,.15,.05)
M21_M22_vol_mu_delta <- seq(0, .6, .2)
...
system.time( 
					...
					,mu_x = sim_params$mu_x				
					,sigma_x = sim_params$sigma_x

...
load("/home/david/Desktop/synthetic_vol_forecasting/simulation_results/simcount_3_savetime_MonMar2718:02:192023_runtime_6.439_hr__grid_size2400_recovery_0.944_permute_0.Rdata")
...
model <- glm(non_NA[,12] ~. , family=binomial(link='logit'),data=non_NA[,c(1:4)])
summary(model)

model <- glm(non_NA[,12] ~.^2 , family=binomial(link='logit'),data=non_NA[,c(1:4)])
summary(model)
...
c6 <- non_NA$vol_sig_noise_ratio < 9.5
#c7 <- TRUE
dom_subset <- non_NA[c1 & c2 & (c3 | c4) & c5 & c6,]
nrow(dom_subset)
p <- mean(dom_subset$`as.integer(vol_model_chosen$QL_adj1 <= vol_model_chosen$QL_adj14)`)

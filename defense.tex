...
\begin{tikzpicture}[
    ...
    shorten > = .5pt, % don't touch arrow head to node
    auto,
...
\node[state] (Target Series) {Target Series};
\node[state] (Donor Pool) [above of = Target Series] {Donor Pool};
%\node[state] (Specified Covariates) [above of = Donor Pool] {Specified Covariates};

\node[rectangle]   (Convex Combination of FE) [above right of=Target Series] {Convex Combination of FE};

\node[state] (Donors) [below right of=Target Series] {$Donors$};
\node[state] (Adjusted Forecast) [right of=Convex Combination of FE] {Adjusted Forecast};

\path[->] (Target Series) edge node {Covariates} (Convex Combination of FE);
\path[->] (Target Series) edge [bend right] node {Unadjusted Forecast} (Adjusted Forecast);
\path[->] (Donor Pool) edge node {Covariates and FE estimates} (Convex Combination of FE);
 \path[->] (Convex Combination of FE) edge node {} (Adjusted Forecast);
...
\begin{frame}
\frametitle{Covariate Profile of a Time Series}
\fontsize{6.6}{7}

...
\subsection{Incorpation of Outside Information is the Norm, Not the Exception}
...
We require that each time series $\y_{i,t}$ is subject to an observed news event following $T^*_i \leq T_{i} + 1$ and before witnessing $T^*_i+1$.  The reasons for this represent the heart and soul of the framework proposed.  We are not proposing a method to correct for unanticipated shocks that have been reflected in the observed data.  Instead, we are proposing a method to correct for shocks for which the qualitative kernel of information is known and the observed quantitative instantiation of that is not yet known. 
...
\textcolor{red}{Clarify for the reader whether the correction function is something that you estimate or simply is an estimator of the correction term.}

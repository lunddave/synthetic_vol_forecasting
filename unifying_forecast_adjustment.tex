...
The term $\x^{T}_{i,t}\lambda_{i,t}$ will be deemed the $\textit{correction term}$ for donor $i$ at time $t$, and the function
...
\textbf{Method 1}: For each source of the $k$ shocks, assemble $n_{j}$ donors and $p_{j}$ covariates, with $j=2,...,n_{j}+1$, and compute weights $\{\pi_{i,j}\}^{n_{j}+1}_{i=2}$.  With these $k$ weight sets, a set of $k$ aggregated shock estimators can be constructed and averaged (or aggregated in some other way).

\textbf{Method 2}: Each of the $k$ shocks can be represented using an application of distance-based weighting, with the vector $\delta$ and the vector $\x_{i,T^{*}_{i}+1}$ can encode a partition of shocks by using zeros in appropriate entries.
...
\end{align*}
We begin our recounting of model-specific cases by recalling \cite{lin2021minimizing}, in which it was established that for a family of AR(1)-X-distributed scalar time series with a common shock distribution, forecast risk can be reduced via a similarity-based adjustment procedure.  An AR(p) is an easy model to work with, in part because it can be consistently estimated via OLS, and an AR(p) can approximate an ARMA(p,q) with arbitrarily-small error through via a truncation of an AR($\infty$) representation.  Setting aside the estimation method, consider a one-step-ahead for an AR(p) at time $t$ with an adjustment estimator, $\hat\alpha_{t}$.  For ease of exposition, we suppress subscripts for donors:

\begin{align}
\hat{y}_{t} = f(\mathcal{F}_{t-1}) = \hat\mu + \sum^{p}_{k=1}\hat\rho_{k}y_{t-k} + \hat{\alpha}_{t}
\end{align}\label{AR(p)-X}
...
Assume
  \begin{enumerate}
    \item For each $i$, $1\leq i \leq n + 1$, let $\{y_{t}\}^{T_{i}}_{i=1}$ follow an AR($p$)-X as laid out in \eqref{AR(p)-X}.
...
Assume
  \begin{enumerate}
   ...
   Then the aggregate estimator $\hat{\alpha_{T^{*}_{1}+1}}$ is consistent as $t \rightarrow \infty$. 

  \end{prop}

\begin{prop}\label{ARIMA_conv_distribution}
  Let $\{\hat y_{1,T_{1}^{*}+r}\}^{h}_{r=1}$ denote the vector of adjusted predictions (adjusted through $h$ steps ahead) in the time series under study.  Assume all conditions listed in Propositions \ref{ARIMA_param_consistency} and \ref{ARIMA_aggregated_alpha}.
   
Then $\{\hat y_{1,T_{1}^{*}+r}\}^{h}_{r=1} \xrightarrow{d} \{ y_{1,T_{1}^{*}+r}\}^{h}_{r=1}$.
  \end{prop}
...
\begin{align}
  &\sigma_{i,t}^{2} = \omega_{i} + \sum^{m_{i}}_{k=1}\alpha_{i,k}a^{2}_{i,t-k} + \sum_{j=1}^{s_{i}}\beta_{i,j}\sigma_{i,t-j}^{2} + \gamma_{i}^{T} \x_{i,t} \text{ }.\label{GARCH_intro}
  \end{align}
...
In the autumn of 2008, as Great Financial Crisis (GFC) gathered steam, the US Federal Reserve reduced its overnight lending rate to banks to between 0 and 25 basis points \citep{GFC}.  Though the US economy (and the globe as a whole) returned to growth by 2009, the US Fed kept interest rates there for more than half a decade.  Only in 2015 did rates begin their gradual and well-telegraphed ascent.  Between 2015 and 2018, the Fed would raise rates at nine meetings, culminating in the December 2018 hike \citep{fedfunds}.  In this real data example, we use the eight hikes preceding December 2018 as donors in order to predict the volatility induced December 2018 rate hike.
...
\begin{enumerate}
  ...
  \item \textbf{Choice of estimator for volatility} We use the sum of squared five-minute log returns of SPY on December 18th, 2018, otherwise known as the Realized Volatility (RV) estimator of volatility \citep{andersen2008realized}, as our proxy.
  \item \textbf{Data Sources} All daily market data is provided via the YahooFinance API available in the quantmod package in R \citep{ryan2015package} and the R package highfrequency \cite{highfrequency}.  The realized volatility estimates of SPY is provided exclusively by the highfrequency R package.  The credit spread is calculated using data Federal Reserve Economic Data (FRED) \citep{boysel2021package}.
\end{enumerate} 

In Figure \ref{fig:HAR_plot}, we observe that our distance-based weighting procedure identifies the December 2015 and March 2018 hikes as the only two relevant donors with respect to the December 2018 meeting.  The fixed effect estimates in the middle panel range from large and positive to modestly negative, in contrast to the estimates in \ref{fig:motivating_piece_convex_combination}.  The GARCH model, under the implementation in the garchx package \text{check this} bounds estimates below by zero.  In the panel on the right, we observe the unadjusted and adjusted predictions, as well as our estimate of the ground truth.  Although the adjusted prediction does miss the volatility spike by a large margin, it appears that this is nearly the best the assembled donors could do, given the donor pool we have to work with.  In that sense, this is a prediction task where the thing-to-predict is far outside the support of the data.
...
\begin{figure}[h!]
  \begin{center}
    \begin{tikzpicture}
      \node[anchor=south west,inner sep=0] at (0,0) {\includegraphics[width=\textwidth]{exponential_sim_plot.png
      }};
      % \draw[red,ultra thick,rounded corners] (7.5,5.3) rectangle (9.4,6.2);
      % \node[draw,text width=4.45cm] at (9.8,3.2) {$\textcolor{blue}{\hat\sigma^{2}_{adjusted} = \hat\sigma^{2}_{unadjusted} + \hat\omega^{*}}$ };
      % \node[draw,text width=2.62cm] at (14.8,2.8) {$\textcolor{blue}{\hat\omega^{*} = \sum^{n+1}_{i=2}\pi_{i}\hat\omega^{*}_{i}}$ };    
  ...
  \end{tikzpicture}
    \caption{The simulated time series experiences a shock...}   \label{fig:exp_shock_plot}
  ...
  \begin{subfigure}{.44\linewidth} 
    \centering
      ...
      \caption{Fixed values: $\sigma{\epsilon}=2$}\label{fig:sim_2}
  \end{subfigure}
...
\begin{proof-of-proposition}[\ref{ARIMA_param_consistency}]
  ...
  The result for each $\hat\alpha$ follows from the consistency proof of the MLE in AR-X models, as established by .
  ...
  \begin{proof-of-proposition}[\ref{ARIMA_aggregated_alpha}]
    \begin{align}
     ...
     \Longrightarrow \lambda^{T}\x_{1} &= \lambda^{T}\sum^{n+1}_{i=2}\weight_{i}\x_{i}  = \sum^{n+1}_{i=2}\weight_{i}\lambda^{T}\x_{i}\tag{by left-multiplication by $\lambda^{T}$}\\
      \Longrightarrow\lambda^{T}\x_{1} &=\sum^{n+1}_{i=2}\pi_{i}(\omega_{i}^{*} - \mu_{\omega^{*}} - u_{i,t})\tag{by substitution and definition of $\omega^{*}_{i}$}\\
      \Longrightarrow\mu_{\omega^{*}} + \lambda^{T}\x_{1} + \sum^{n+1}_{i=2}\pi_{i}u_{i,t} &= \sum^{n+1}_{i=2}\pi_{i}\omega_{i}^{*} \tag{by moving  terms to the left-hand side}
      \end{align} 
  \end{proof-of-proposition}
  ...
  \begin{proof-of-proposition}[\ref{ARIMA_conv_distribution}]
    ...
    ($\hat\rho_{1},...,\hat\rho_{p}, \hat\alpha_{i,T^{*}+1}$) is consistent at $t\rightarrow \infty$.

    Next consider the prediction function for an AR($p$)-X,

    \begin{align*}
      \hat{y}_{t} = \hat{f}(\mathcal{F}_{t-1}) = \hat\mu + \sum^{p}_{k=1}\hat\rho_{k}y_{t-k} + \hat{\alpha}_{t}
      \end{align*}

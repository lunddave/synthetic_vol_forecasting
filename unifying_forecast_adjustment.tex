...
Consider the leave-one-out method used on pairs of donors and covariates in \cite{lundquist2024volatility}.  By averaging over $(n+1)\cdot (p+1)$ predictions, we average over $n \cdot p$ convex combinations (where, when a donor is ommitted, its weight can be imputed as zero).  
...
\subsection{The essential elements of similarity-based parameter correction}

\textcolor{red}{Should I use OLS as an example throughout all of these?}

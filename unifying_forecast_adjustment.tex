...
\begin{enumerate}
  ...
  \item Distinction between regular forces J and its complement, K 
...
\begin{forest}
  ...
  text width=3cm,%
    ...
    [Is there a break in the DGP between T* and T*+1?
        [Do you have information outside of the “regular forces” \citep{castle2011forecasting}, %breaking news or high-frequency data, e.g. Google Trends data?
            [Can we use the info outside “regular forces” to forecast (as opposed to merely spotting the break)?
                [Do you have a parametric model for how info outside of ``regular forces” will figure at T* + h?
                  % [What other time series have been governed by that parametric model, and how can we extract information from them?  Which other series are most relevant or most similar?  How do we determine similarity – quant or qual?  Once you know what’s similar, how do you use that?
                  % ]
                  % [The info that helped you spot the break can be used to assemble series that have experienced breaks under similar circumstances.  The forecast residuals or losses from those then provide a basement for model adjustment for the time series under study.  Without a parametric model, your options are limited but not empty.  With appropriate assumptions on the DGP pre-break and post-break, assumptions on the forecast functions and misses, we can still do things like aggregate residuals and bound forecast loss.]
                  [some extra]
                  [some extra again]
                ]
                [No problem. Use “regular forces” to do the model adjustment for the post-break period.  Examples of this would include refitting one’s data to use Markov-switching models – i.e. using only the series itself.
                ]
            ]
            [Then at least one post-break data point must be observed.
            ]
        ]
        [\cite{clements1998forecasting} talk about this a lot]
    ]
\end{forest}
}%

...
\end{align}
(\ref{plugin}) is, to put it lightly, a complicated estimator with potentially dubious properties.  To begin, (\ref{plugin}) is defined only when the term inside the logarithm is positive for each $t\geq T^{*}_{i}+2$.  We could drop any summands for which that term is nonpositive and then calculate the average over that subset.

Could it be, then, that aggregating the prediction residuals from the donors could provide us better predictions in the time series under study?\\

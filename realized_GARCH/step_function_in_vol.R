...
inn_low_vol <- rnorm(n/2, sd = .1)
inn_high_vol <- rnorm(n/2, sd = .2)

step_function_series_low_vol <- garchxSim(n/2, innovations = inn_low_vol)
step_function_series_high_vol <- garchxSim(n/2, innovations = inn_high_vol)
series <- c(as.vector(step_function_series_low_vol),as.vector(step_function_series_high_vol))
plot.ts(series)

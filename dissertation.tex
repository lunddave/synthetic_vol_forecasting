...
\degreeyear{2025}
\committee{
    Assistant Professor Daniel Eck, Chair and Director of Research\\
    Assistant Professor Gökçe Dayanıklı \\
...
A relevant limitation, though relatively rare limitation, is that ground truth may not exist necessarily for any of the $n$ donor units. Therefore, errors and losses may be calculable in only a proper subset of size $k < n$, raising concerns about not only the sample size but also the representativeness of the subsample.  However, the lack of ground truth does not mean that all is lost.  One can still generate the predictions and ask `are any of these predictions so incredible that I have reason to doubt the viability or appropriateness of this donor pool?'.
...
It should be noted, however, that even when the minimizer is unique, finding that unique minimizer may require numerical methods.  In the event that one's preferred numerical optimization routine fails to arrive at a single solution on the simplex in repeated runs of the algorithm, some sort of forecast combination might be advised.  We discuss forecast combination later in this paper in the context of robustifying against misspecification.
    ...
    \begin{center}
      \captionof{table}{\label{tab:Tcr_1}}
...
The additional avenues for simulations to explore post-shock volatility forecasting are none too few.  One such idea is how poorly a practitioner might forecast if they employ the approach of \cite[][]{lin2021minimizing} when ARCH effects are present.  The simple and slightly unsatisfying answer is arbitrarily poor.  One can draw up AR(1), AR($p$), ARMA($p,q$) (and so on) examples that such that, when estimated using an insufficient number of parameters, prediction performance suffers.  One can draw up a GARCH($m,s$) process and then estimate it with any of AR(1), AR($p$), ARMA($p,q$) such that the prediction fails badly.  Perhaps one guiding principle that would help properly scope such a research area is identifying reasonable cases of misspecification and reasonable attempts to forecast under those settings.
...
As an additional note, we would add that here, an analysis via the permutation method was unfortunately not available to us, and for at least two reasons.  First, as we have discussed previously, the $\binom{n+1}{2}$ pairwise relationships among the full pool of units will not necessarily have the property of symmetry, i.e. that if $A$ is a suitable donor for $B$, then $B$ is a suitable donor for $A$.  This is relevant, at the very least, in the case of the three Brexit donors, which might be good donors for neither events prior to 2016 nor events outside of the UK.  Second, due to the difficulty of procuring high-frequency equity price data, we lack realized volatility estimates of the volatility for several donors in this example, which greatly impedes our ability to approximate the ground truth, calculates errors and losses, and make a loss-based evaluation of the donor pool.
...
\begin{figure}[H]
  ...
  \begin{table}[h]
    ...
    \begingroup\fontsize{6.5pt}{6.5pt}\selectfont
    \begin{tabular}{ccc}
...
\begin{proof-of-proposition}[\ref{decay_prop}]
  ...
  \begin{proof-of-lemma}[\ref{lemma_ref}]
    ...
    \begin{proof-of-proposition}[\ref{omega_consistency}]
...
Now, since each $\hat\omega^{*}_{i}$ converges in probability to $\omega^{*}_{i}$, by Slutsky, any finite linear combination of $\hat\omega^{*}_{i}$ converges in probability to the corresponding finite linear combination of $\omega^{*}_{i}$.  Hence, $\sum^{n+1}_{i=2}\pi_{i}\hat\omega_{i}^{*}$ converges in probability to the right-hand side of the final line.  By setting $u_{i,t}$ to zero for all shock points and all $i$, we then have $\omega_{1}^{*} = \mu_{\omega^{*}} + \delta^{T}\x_{1} $ on the left-hand side.  Without taking the $u_{i,t}$ to be zero at the shock points, however, we nevertheless have convergence in distribution.  The proof is as follows.  For any collection of $n+1$ i.i.d variables, any convex combination of any $n$ of them is equal in distribution to the one left out.  Therefore, the left-hand side of the final line is equal in distribution to $\omega_{1}$, as desired.
  ...
  \end{tikzpicture}
  \caption{The 3-Simplex, $\Delta^{3}$, where hypothetical minimizer is a convex combination of Donors 1 and 4.  Graphic adapted from \cite[][]{tetrahedron}.}
  \end{figure}
  ...
  We would like to pause here to discuss the results just stated.  From \ref{omega_consistency} onward, several results have been established using a condition that implies strong stationarity of the time-varying fixed effects.  The condition requires that a given time series has only one volatility shock in its entire existence, which could be viewed as implausible or unrealistic.
 It is first important to note that the time-varying fixed effects in \ref{core_specification} nest the model found in \cite[][]{lin2021minimizing}, which can be characterized as having zeroed-out all of the fixed effects besides the one at $T^{*}+1$.  What would be welcomed in this area is both more lightweight notation as well as efforts to permit more than one shock in the volatility series on a unit.  Such efforts will elevate the method under study from mere empirical performance to a sturdier theoretical foundation. 
